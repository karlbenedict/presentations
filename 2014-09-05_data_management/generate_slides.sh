...
find . -iname "*.md" | sort -r | xargs -I \{\} /usr/local/bin/pandoc --section-divs --slide-level 3 -c slides.css  -c slide_specific.css --self-contained --standalone -t dzslides \{\} -o \{\}.slides.html
find . -iname "*.md" | sort -r | xargs -I \{\} /usr/local/bin/pandoc --section-divs --slide-level 3 -c slides.css  -c slide_specific.css --standalone -t dzslides \{\} -o \{\}.slides.linked.html
